...
__PROXY_USERPWD__   = 'mgray02:4JwRNV8G' # proxy password
__THREAD_NUMBER_    = 1 # Thread number
...
def wait(sleep_time, log_text=''):
    ...
    print log_text , ":", rand_sleep_time # log
    sleep(rand_sleep_time)  # sleep 
# end wait function
#******************************************************************
# define init proxies
def init_proxies(idx):
    proxy_host = "46.101.204.69"
    # proxy_host = proxies.proxies[idx].split(':')[0]
    proxy_port = "1080"
    # proxy_port = proxies.proxies[idx].split(':')[1]
    proxy_username = "ofirseo"
    # proxy_username = __PROXY_USERPWD__.split(':')[0]
    proxy_password = "a1qs2wd3ef4r"
    # proxy_password = __PROXY_USERPWD__.split(':')[1]
