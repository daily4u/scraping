...
def init_chrome_driver(idx):
    ...
    ind = idx + 10
    # proxyauth_plugin_path = config.init_proxies(ind)
...
def main():
    ...
    for i in range(5):
        drivers.append(init_chrome_driver(i))
...
def parse_detail(_url, ind):
    global active_threads
    global drivers
    driver = drivers[int(ind) % 5]
    # driver[ind ] = init_chrome_driver(int(ind) % 20)
    driver.get(_url)
    time.sleep(5)
    print "++++++++++++++++1+++++++++++++++++"

    try:
        driver.WebDriverWait(driver,30).until(EC.presence_of_element_located((By.XPATH, '//a[@title="All Investments"]')))
    except:
        pass
    print "++++++++++++++++++++++++++++++++++"
    item = []
    ...
    for row in overviews:
        ...
        elif ttt == "Founded:" or ttt == "Born:":
            founded = row.x("following-sibling::dd[1]/text()").trim()
        elif ttt == "Aliases:" or ttt == "Gender:":
        ...
        elif ttt == "Regions:" or ttt == "Location:":
